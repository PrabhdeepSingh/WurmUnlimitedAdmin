...
class PLAYER
{
  ...
  function GetPlayers($playerID = "")
  {
    try
    {
      ...
      {
        $sql = $this->_playerDB->QueryWithOutBinds("SELECT WURMID, NAME, POWER FROM PLAYERS ORDER BY NAME");
      ...
      if(!empty($params))
      {
        ...
        if($user != false)
        {
          ...
          for($i = 0; $i < sizeof($itemIDs); $i++)
          {
            ...
            {
              $result = array("success" => false, "error" => array("message" => "Unable to add item to players inventory."));
            }
            else
            {
